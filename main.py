...
Now supports:
- Hybrid optimization (evolutionary + gradient descent)
...
Usage:
    ...
    python main.py --hybrid           # Hybrid evolutionary + gradient training
    python main.py --fast             # Fast mode with optimizations
...
# Hybrid training settings (evolutionary + gradient)
USE_HYBRID = False     # Set to True for hybrid evolutionary + gradient training
POPULATION_SIZE = 20   # Population size for evolutionary optimization
ENABLE_DYNAMIC = True  # Enable dynamic structure modification
...
def main():
    ...
    parser.add_argument('--hybrid', action='store_true', help='Use hybrid evolutionary + gradient training')
    parser.add_argument('--fast', action='store_true', help='Use fast mode with speed optimizations')
    parser.add_argument('--dynamic', action='store_true', help='Enable dynamic structure modification')
    args, _ = parser.parse_known_args()
    
    use_hybrid = USE_HYBRID or args.hybrid
    enable_dynamic = ENABLE_DYNAMIC or args.dynamic
    
    # Create model
    print(f"\nCreating model...")
    print(f"  - Max depth: {MAX_DEPTH}")
    print(f"  - Candidates: {N_CANDIDATES}")
    print(f"  - Mode: {'Hybrid (Evolutionary + Gradient)' if use_hybrid else 'Gradient Descent'}")
    print(f"  - Dynamic structure: {'Enabled' if enable_dynamic else 'Disabled'}")
    
    if use_hybrid:
        # Use hybrid training
        from hybrid_trainer import HybridSymbolicTrainer, HybridTrainerConfig
        ...
        config = HybridTrainerConfig(
            ...
            enable_dynamic_structure=enable_dynamic,
            early_stopping_patience=min(500, N_EPOCHS // 4)
        )
        
        trainer = HybridSymbolicTrainer(model, config)
        
        print(f"\nTraining model with hybrid optimization ({N_EPOCHS} epochs)...")
