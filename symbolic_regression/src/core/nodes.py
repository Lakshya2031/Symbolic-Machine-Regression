...
class SymbolicNode(nn.Module, ABC):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Evaluate the node on input tensor x of shape (batch, n_features)."""
        pass
    
    @abstractmethod
    def get_complexity(self) -> torch.Tensor:
        """Return the complexity cost of this subtree."""
        pass
    
    @abstractmethod
    def to_expression(self, var_names: List[str]) -> str:
        """Convert to human-readable expression (continuous weights)."""
        pass
    
    @abstractmethod
    def simplify(self, var_names: List[str]) -> str:
        """Convert to simplified expression (discretized operators)."""
        pass
...
class ConstantNode(SymbolicNode):
    ...
    def simplify(self, var_names: List[str]) -> str:
        ...
        for nice_val in [0, 1, -1, 2, -2, 0.5, -0.5, 3, -3]:
            if abs(val - nice_val) < 0.05:
                return f"{nice_val:g}"
        return f"{val:.4f}"
...
class WeightedInputNode(SymbolicNode):
    ...
    Uses softmax weights to create a differentiable selection over
    input features. This allows the model to learn which feature
    is most relevant at this position in the expression tree.
    
    Modes:
        - "soft": Differentiable weighted combination
        - "hard": Select single feature (argmax)
    """
    ...
    def __init__(self, n_features: int, mode: str = "soft"):
        ...
        # Learnable logits for feature selection
        self.feature_logits = nn.Parameter(torch.zeros(n_features))
        
        # Learnable scale factor (constrained to reasonable range)
        self.scale = nn.Parameter(torch.ones(1))
        
        # Max scale to prevent explosion
        self.max_scale = 10.0
    
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Constrain scale to reasonable range
        scale = torch.clamp(self.scale, min=-self.max_scale, max=self.max_scale)
    ...
    def get_dominant_feature(self) -> int:
        """Get the feature with highest weight."""
...
class UnaryOpNode(SymbolicNode):
    ...
    The operator is selected via softmax weights over all available
    unary operators, making the selection differentiable.
    """
...
class PowerNode(SymbolicNode):
    ...
    The exponent is a learnable parameter, allowing the model
    to discover arbitrary power relationships.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Use exp(p * log(|x|)) for differentiability
