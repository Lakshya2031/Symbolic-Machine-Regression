...
Key features:
- Fixed or dynamic expression depth/complexity
...
- Support for evolutionary and hybrid optimization
- Dynamic structure modification during training
"""
...
class SymbolicRegressor(nn.Module):
    ...
    # =========================================================================
    # Methods for Dynamic Structure and Evolutionary Optimization
    # =========================================================================
    
    def add_candidate(self, structure: str = "mixed", depth: Optional[int] = None) -> int:
        """
        Add a new candidate expression to the model.
        
        Args:
            structure: Structure type for new candidate
            depth: Depth for new candidate (defaults to max_depth)
            
        Returns:
            Index of new candidate
        """
        if depth is None:
            depth = self.max_depth
        
        new_expr = SymbolicExpression(self.n_features, depth, structure)
        self.candidates.append(new_expr)
        
        # Expand candidate logits
        new_logits = torch.zeros(len(self.candidates))
        new_logits[:-1] = self.candidate_logits.data
        self.candidate_logits = nn.Parameter(new_logits)
    ...
    def remove_candidate(self, index: int) -> bool:
        """
        Remove a candidate expression by index.
        
        Args:
            index: Index of candidate to remove
            
        Returns:
            True if successful, False if cannot remove (e.g., only one left)
        """
        ...
        new_logits = torch.cat([
            self.candidate_logits.data[:index],
            self.candidate_logits.data[index + 1:]
        ])
        self.candidate_logits = nn.Parameter(new_logits)
    ...
    def prune_weak_candidates(self, threshold: float = 0.05) -> int:
        """
        Remove candidates with weights below threshold.
        
        Args:
            threshold: Minimum weight to keep candidate
            
        Returns:
            Number of candidates removed
        """
        weights = self.get_candidate_weights().detach()
        to_remove = []
        
        for i, w in enumerate(weights):
            if w.item() < threshold and len(self.candidates) - len(to_remove) > 1:
                to_remove.append(i)
        
        # Remove in reverse order to preserve indices
        removed = 0
        for i in reversed(to_remove):
            if self.remove_candidate(i):
                removed += 1
        
        return removed
    
    def clone(self) -> 'SymbolicRegressor':
    ...
    def randomize_parameters(self, std: float = 1.0):
        """
        Randomize all parameters (for evolutionary initialization).
    ...
    def perturb_parameters(self, std: float = 0.1):
        """
        Add random noise to all parameters (for mutation).
    ...
    def get_structure_info(self) -> Dict[str, Any]:
        ...
        info = {
            ...
            "dominant_candidate": int(torch.argmax(self.candidate_logits).item()),
        ...
        for i, cand in enumerate(self.candidates):
            info["structures"].append({
                "index": i,
                "weight": weights[i],
                "type": cand.structure_type,
                "depth": cand.max_depth
            })
        
        return info
    
    def blend_with(self, other: 'SymbolicRegressor', alpha: float = 0.5) -> 'SymbolicRegressor':
        """
        Create a new model by blending parameters with another model.
        
        Args:
            other: Another SymbolicRegressor to blend with
            alpha: Blending weight (0 = all self, 1 = all other)
            
        Returns:
            New blended model
        """
        if self.n_features != other.n_features:
            raise ValueError("Models must have same number of features")
        
        blended = self.clone()
        
        with torch.no_grad():
            for (name, param), (_, other_param) in zip(
                blended.named_parameters(),
                other.named_parameters()
            ):
                if param.shape == other_param.shape:
                    param.data = (1 - alpha) * param.data + alpha * other_param.data
